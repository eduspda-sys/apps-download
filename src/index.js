...
export default {
  async fetch(request, env) {
    ...
    if (path === "/") return new Response(storePage(), { headers: HTML_HEADERS });
    if (path === "/admin") return new Response(adminPage(), { headers: HTML_HEADERS });
...
<div class="box"><div class="drop"><strong>Enviar arquivo</strong><div class="sub">APK, imagem ou outro arquivo</div><div class="controls"><input id="file" type="file"><select id="folder"><option value="apks">APK</option><option value="images">Imagem</option><option value="files">Outro arquivo</option></select><button onclick="upload()">Enviar</button></div><div id="status" class="status"></div></div></div>
...
async function load(){const r=await fetch('/api/files');const a=await r.json();const el=$('#list');if(!a.length){el.innerHTML='<div class="sub">Nenhum arquivo enviado.</div>';return}el.innerHTML=a.map(x=>'<div class="row"><div><div class="name">'+esc(x.key)+'</div><div class="small">'+fmt(x.size)+'</div></div><button onclick="copyLink(\''+js(x.url)+'\')">Copiar link</button><button class="danger" onclick="del(\''+js(x.key)+'\')">Excluir</button></div>').join('')}
async function upload(){const f=$('#file').files[0];if(!f){$('#status').textContent='Selecione um arquivo.';return}const d=new FormData();d.append('file',f);d.append('folder',$('#folder').value);$('#status').textContent='Enviando...';const r=await fetch('/api/upload',{method:'POST',body:d});const j=await r.json();if(!r.ok){$('#status').textContent=j.error||'Falha no upload';return}$('#status').textContent='Enviado com sucesso.';$('#file').value='';await load()}
async function del(k){if(!confirm('Excluir este arquivo?'))return;await fetch('/api/files/'+encodeURIComponent(k),{method:'DELETE'});load()}
async function copyLink(u){await navigator.clipboard.writeText(location.origin+u);alert('Link copiado')}
